...
- account for every collected report-ready source section before drafting starts
...
10. Build a per-source coverage manifest that classifies every report-ready bundle section as `main_body`, `appendix`, `omitted_with_reason`, `unavailable`, or `known_gap`.
11. Draft the report section by section using `nexon-combined-monthly-report` for section choice, module shape, visual expectations, metric usefulness, commentary quality, and appendix discipline.
12. Render the draft artifact in the format required by the selected customer/report flow.
13. Delegate to the `reporting-data-validation` sub-agent before any review handoff or delivery step.
14. Pass that sub-agent:
...
15. Require `reporting-data-validation` to return:
    - verdict
    - claim-level findings
    - corrections made
    - severity for each finding
    - corrected wording or corrected metrics where possible
16. Let the sub-agent fix any in-scope data issues it can prove from source evidence, then require it to re-run validation before returning.
17. If the data-validation verdict is not a pass, revise any remaining issues and re-run `reporting-data-validation` before any visual QA or review handoff.
18. If the output is a customer-facing HTML report or PPT/slide deck, delegate to the `reporting-qa` sub-agent after data validation passes.
19. Pass the sub-agent the rendered artifact reference plus any preview assets needed for visual inspection, such as slide images or HTML preview captures.
20. Require the sub-agent to return:
    - verdict
    - location-based findings
    - corrections made
    - severity for each finding
    - recommended fixes
21. Let the sub-agent fix any in-scope visual issues it can safely resolve, then require it to re-run QA before returning.
22. If the QA verdict is not a pass, revise any remaining artifact issues and re-run `reporting-qa` on the affected sections or slides and then on the final artifact skim.
23. If SDM review is required, send the review handoff only after the draft artifact is verified.
24. Support revision requests without re-collecting source data unless a true data gap is confirmed.
25. Finalize and deliver the approved artifact.
...
- Before drafting, enumerate the collected report-ready sections for each source bundle and keep a source-coverage manifest for the run.
- No report-ready bundle section may be silently dropped. Every collected section must be:
  - used in the main body
  - moved to appendix or detail evidence
  - marked as unavailable because the source attempt failed
  - marked as a source-side known gap
  - omitted with an explicit reason tied to customer value, duplication, or presentation density
...
- Prefer the canonical `bundle.sections` object when a source bundle provides one.
- If a source bundle exposes both canonical `sections` and backward-compatible top-level aliases, treat `sections` as the source of truth for coverage accounting.
- Do not force every possible section into the report. Use the modules that add value for that reporting period.
- "Do not force every possible section" does not permit silent omission. If a report-ready section is not shown, record why and where the reader can find the remaining evidence, if anywhere.
