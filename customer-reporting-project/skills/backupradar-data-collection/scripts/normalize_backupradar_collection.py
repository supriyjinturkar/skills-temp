...
from br_pipeline import normalize_backupradar_snapshot
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = args["output_path"] or run_paths["backup_file"]
    write_json_file(output_path, backup)
    sys.stdout.write(f"{json.dumps({'ok': True, 'output_path': output_path, 'dataset': backup['dataset']})}\n")
    return 0
