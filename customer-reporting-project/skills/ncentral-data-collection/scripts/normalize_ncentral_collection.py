...
from nc_pipeline import normalize_ncentral_snapshot
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = args["output_path"] or run_paths["ncentral_normalized_file"]
    write_json_file(output_path, normalized)
    sys.stdout.write(f"{json.dumps({'ok': True, 'output_path': output_path, 'dataset': normalized['dataset']})}\n")
    return 0
