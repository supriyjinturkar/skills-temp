...
- Logo minimum 100px wide. Clear space = height of the `x` glyph on all sides.
...
# Secondary palette
WILDCARD  = RGBColor(0x00, 0xC9, 0x82)  # #00c982 - CTAs and accents
...
# H1 title
tx_box = slide.shapes.add_textbox(Inches(0.4), Inches(2.5), Inches(9), Inches(2))
...
## PNG Decode Step For Fleet Sandbox Runs

When Fleet copies this skill into its sandbox, it can copy `/skills/nexon-brand/assets/logo.png` as UTF text instead of binary, which corrupts the image. In that environment, open that copied file as text, decode the Base64 content it contains, and then use the reconstructed PNG in PowerPoint:

```python
import base64
from pathlib import Path

source = Path("/skills/nexon-brand/assets/logo.png")
target = Path("/workspace/nexon-logo.png")
...
print(f"Decoded {len(decoded)} bytes -> {target}")
```

Use this decode step for Fleet sandbox runs or any other workflow where the copied asset file is corrupted. Outside that case, prefer the checked-in binary `logo.png`.
...
- `python-pptx` does not auto-embed the theme palette. Set `RGBColor` values explicitly on each shape instead of relying on theme slot inheritance.
- Logo PNG must be the official white-on-black asset. Never approximate with a text shape or recoloured image.
- Gateway yellow (`#ffbf00`) looks fine on black but must never be the logo background.
